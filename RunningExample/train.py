...
learning_rate = 0.000001; decay=0.0; inf=1
...
in_dims = 1
model = Sequential()
model.add(Dense(32, activation="tanh", input_shape=(None, 1)))
model.add(Dense(1, activation="linear"))

#width = 128
#model = Sequential()
#model.add(Dense(width, activation="tanh", input_shape=(None, 1)))
#model.add(Dense(1, activation="linear"))
loss = tf.keras.losses.MeanSquaredError()

bayes_model = opt.compile(model, loss_fn=loss, epochs=35, learning_rate=learning_rate,
                          batch_size=50, linear_schedule=True, mode='regression',
                          decay=decay, robust_train=rob, inflate_prior=inf,
                          burn_in=3, steps=25, b_steps=20)

...
class PriorHamiltonianMonteCarlo(optimizer.Optimizer):
    def __init__(self):
        super().__init__()
    ...
    def compile(self, keras_model, loss_fn, batch_size=64, learning_rate=0.15, decay=0.0,
        ...
        self.path = kwargs.get('path', "HMC_Posterior")
    ...
    def sample(self, features, labels, lrate, features_l=None, features_u=None, constraint=False):
        ...
        if(constraint):
            self.constraint_step(features, labels, lrate/2.0, features_l, features_u)
        else:
        ...
        for i in trange(steps, desc="Numerical Integration"):
            ...
            if(constraint):
                self.constraint_step(features, labels, lrate, features_l, features_u)
            else:
        ...
        if(random.uniform(0,1) < met_const):
            ...
            if(not self.burning_in_chain):
                if not os.path.exists(self.path+"/samples"):
                    os.makedirs(self.path+"/samples")
                print("saving with first value: ", np.asarray(copy.deepcopy(self.q))[0][0][0])
                np.save(self.path+"/samples/sample_%s"%(len(self.posterior_samples)), np.asarray(copy.deepcopy(self.q)))
                arr = np.load(self.path+"/samples/sample_%s.npy"%(len(self.posterior_samples)), allow_pickle=True)
                print("loaded with first value: ", arr[0][0][0])
    ...
    def constraint_step(self, features, labels, lrate, features_l, features_u):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if(self.mode == 'classification'):
                ...
                logit_l, logit_u = analyzers.IBP(self, features-self.epsilon, features+self.epsilon,
                                                 weights=self.model.get_weights())
                worst_case_l = self.model.layers[-1].activation(logit_l)
                worst_case_u = self.model.layers[-1].activation(logit_u)
                output_l = (self.robust_lambda * predictions) + ((1-self.robust_lambda) * worst_case_l)
                output_u = (self.robust_lambda * predictions) + ((1-self.robust_lambda) * worst_case_u)
                loss = self.loss_func(labels, output_l) + self.loss_func(labels, output_u)
                #loss = self.loss_func(labels, output_u)
            else:
                logit_l, logit_u = analyzers.IBP(self, features-self.epsilon, features+self.epsilon,
                                                 weights=self.model.get_weights())
                worst_case_l = self.model.layers[-1].activation(logit_l)
    ...
    def train_chain(self, X_train, y_train, X_test=None, y_test=None):
        self.model.set_weights(self.prior_sample())
    ...
    def finalize_chain(self):
        ...
        print("Finalized chain", len(self.num_rets), len(self.posterior_samples))
            
    def draw(self):
        index = np.random.choice(range(len(self.posterior_samples)))
